...
class WasmRuntime {
  ...
  WasmRuntime._init() : _lib = _loadDynamicLib() {
    ...
    if (_Dart_InitializeApiDL(NativeApi.initializeApiDLData) != 0) {
      throw _WasmErrorRuntimeImpl('Failed to initialize Dart API');
    }
    _engine = _engine_new();
  ...
  int getReturnType(Pointer<WasmerFunctype> funcType) {
    ...
    if (rets.ref.length == 0) {
      ...
      throw _WasmErrorRuntimeImpl('Multiple return values are not supported');
    }
  ...
  T _checkNotEqual<T>(T x, T y, String errorMessage) {
    if (x == y) {
      throw _WasmErrorRuntimeImpl('$errorMessage\n${_getLastError()}'.trim());
    }
    return x;
  }
}
