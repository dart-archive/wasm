...
WasmRuntime wasmRuntimeFactory(
  ...
  Error Function(String message) errorFactory,
) {
  return WasmRuntime._(lib, trapExceptionFactory, errorFactory);
}
...
class WasmRuntime with _WasmRuntimeGeneratedMixin {
  ...
  final Error Function(String message) errorFactory;

  WasmRuntime._(
    DynamicLibrary lib,
    this.trapExceptionFactory,
    this.errorFactory,
  ) {
    initBindings(lib);
    if (_Dart_InitializeApiDL(NativeApi.initializeApiDLData) != 0) {
      throw errorFactory('Failed to initialize Dart API');
    }
    _engine = _engine_new();
  ...
  int getReturnType(Pointer<WasmerFunctype> funcType) {
    ...
    if (rets.ref.length == 0) {
      ...
      throw errorFactory('Multiple return values are not supported');
    }
